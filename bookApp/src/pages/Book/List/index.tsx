...
const BookList: React.FC = () => {
  ...
  const [currentRow, setCurrentRow] = useState<BookItem>();
  ...
  const fetchCategories = async () => {
    try {
      const response = await request('/api/book/categories', {
        method: 'GET',
      });
  ...
  const handleAdd = async (fields: BookItem) => {
    const hide = message.loading('正在添加');
    try {
      await request('/api/book/add', {
        method: 'POST',
        data: fields,
      });
      hide();
  ...
  const handleUpdate = async (fields: BookItem) => {
    const hide = message.loading('正在更新');
    try {
      await request('/api/book/update', {
        method: 'PUT',
        data: fields,
      });
      hide();
  ...
  const handleRemove = async (id: number) => {
    ...
    try {
      await request(`/api/book/delete/${id}`, {
        method: 'DELETE',
      });
      hide();
  ...
  const handleExtract = async (id: number) => {
    ...
    try {
      const response = await request(`/api/book/extract/${id}`, {
        method: 'POST',
      });
      hide();
      ...
      if (response.code === 20001) {
        ...
        const interval = setInterval(async () => {
          try {
            const resultResponse = await request(`/api/book/extract/result/${taskId}`, {
              method: 'GET',
            });
  ...
  // 导入章节到ES
  const handleImportToEs = async (record: BookItem) => {
    const hide = message.loading('正在导入...');
    try {
      await request(`/api/es/books/import/book/${record.id}`, {
        method: 'POST',
      });
      hide();
  ...
  const handleExcelUpload = async (file: File) => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          const response = await request('/api/book/import/excel', {
            method: 'POST',
            data: formData,
          });
          ...
          if (response.code === 20001) {
            ...
            const interval = setInterval(async () => {
              try {
                const resultResponse = await request(`/api/book/import/status/${taskId}`, {
                  method: 'GET',
                });
  ...
  const handleGetOpac = async (id: number) => {
    ...
    try {
      const response = await request(`/api/book/opac/${id}`, {
        method: 'POST',
      });
      hide();
      ...
      if (response.code === 20001) {
        ...
        const interval = setInterval(async () => {
          try {
            const resultResponse = await request(`/api/book/opac/status/${taskId}`, {
              method: 'GET',
            });
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (response.code === 20001) {
            ...
            const interval = setInterval(async () => {
              try {
                const resultResponse = await request(`/api/book/opac/batch/status/${taskId}`, {
                  method: 'GET',
                });
  ...
  const handleDeleteIndex = async (record: BookItem) => {
    try {
      const response = await request(`/api/es/books/book/${record.id}`, {
        method: 'DELETE',
      });
      if (response.code === 0) {
  ...
  const columns: ProColumns<BookItem>[] = [
    {
      title: '书名',
      dataIndex: 'title',
      width: 200,
  ...
  return (
    <PageContainer>
      ...
      <BookForm
        ...
        onFinish={async (values: BookItem) => {
          let success;
          if (currentRow?.id) {
