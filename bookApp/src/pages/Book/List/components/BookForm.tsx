...
import { getPreviewUrl } from '@/services/book';
...
const BookForm: React.FC<BookFormProps> = (props) => {
  ...
  // 加载图书的主题标签
  const loadBookTopics = async (bookId: number) => {
    try {
      const response = await request(`/api/topic/book/${bookId}/paths`, {
        method: 'GET',
      });
  ...
  // 加载图书的自定义标签
  const loadBookTags = async (bookId: number) => {
    try {
      const response = await request(`/api/topic/book/${bookId}/tags`, {
        method: 'GET',
      });
      if (response.code === 0 && Array.isArray(response.data)) {
  ...
  const loadTopicTree = async () => {
    ...
    try {
      ...
      if (response.code === 0 && Array.isArray(response.data)) {
        ...
        // 如果有图书ID，加载图书的主题标签
        if (values?.id) {
          await loadBookTopics(values.id);
        }
      } else {
  ...
  const handleTopicChange = async (newTopicIds: string[]) => {
    ...
    try {
      ...
      for (const topicId of toDelete) {
        await request(`/api/topic/book/${values.id}/topic/${topicId}`, {
          method: 'DELETE',
        });
      }
  ...
  const handleTagChange = async (newTags: string[]) => {
    ...
    try {
      ...
      for (const tagId of toDelete) {
        await request(`/api/topic/book/${values.id}/topic/${tagId}`, {
          method: 'DELETE',
        });
      }
  ...
  // 添加处理表单提交的函数
  const handleSubmit = async (formValues: BookItem) => {
    try {
      const processedValues: BookItem = {
        ...formValues,
      };
              ...
              { required: true, message: '请输入书名' },
              {
                validator: async (_, value) => {
                  if (value && (value.includes(':') || value.includes('：'))) {
                    return Promise.reject(new Error('书名不得包含冒号，请用空格代替'));
                  }
                },
              },
            ]}
