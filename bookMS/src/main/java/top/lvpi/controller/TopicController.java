...
public class TopicController {
    ...
    @PostMapping("/book/{bookId}/topic/{topicId}")
    @Operation(summary = "为书籍添加主题标签、自定义标签")
    public BaseResponse<Long> addBookTopic(@PathVariable Long bookId, @PathVariable Long topicId) {
        return ResultUtils.success(bookTopicService.addBookTopic(bookId, topicId));
    }

    @DeleteMapping("/book/{bookId}/topic/{topicId}")
    @Operation(summary = "删除书籍的指定主题标签、自定义标签")
    public BaseResponse<Boolean> removeBookTopic(@PathVariable Long bookId, @PathVariable Long topicId) {
        return ResultUtils.success(bookTopicService.removeBookTopic(bookId, topicId));
    }

    @DeleteMapping("/book/{bookId}/topics")
    @Operation(summary = "删除书籍的所有主题标签、自定义标签")
    public BaseResponse<Boolean> removeAllBookTopics(@PathVariable Long bookId) {
        return ResultUtils.success(bookTopicService.removeAllBookTopics(bookId));
    }

    @GetMapping("/book/{bookId}")
    @Operation(summary = "获取书籍的所有主题及其层级关系（树形结构）")
    public BaseResponse<List<TopicTreeDTO>> getBookTopics(@PathVariable Long bookId) {
        return ResultUtils.success(topicService.getTopicTreeByBookId(bookId));
    }

    @GetMapping("/book/{bookId}/paths")
    @Operation(summary = "获取书籍的所有主题及其层级关系（格式化）")
    public BaseResponse<List<TopicPathDTO>> getBookTopicPaths(@PathVariable Long bookId) {
        return ResultUtils.success(topicService.getBookTopicPaths(bookId));
    }
    
    @GetMapping("/book/{bookId}/tags")
    @Operation(summary = "获取书籍的所有自定义标签，不包括主题标签")
    public BaseResponse<List<Topic>> getBookTopicTags(@PathVariable Long bookId) {
        return ResultUtils.success(topicService.getBookTopicTags(bookId));
    }
