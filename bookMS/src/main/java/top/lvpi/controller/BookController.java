...
public class BookController {
    ...
    @PostMapping("/add")
    public BaseResponse<Integer> addBook(@RequestBody BookAddRequest bookAddRequest) {
        if (bookAddRequest == null) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        String title = bookAddRequest.getTitle();
        if (StringUtils.isBlank(title)) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "书名不能为空");
        }
        try {
            int result = bookService.addBook(bookAddRequest);
    ...
    public BaseResponse<Boolean> deleteBook(@PathVariable("id") Long id) {
        if (id <= 0) {
    ...
    public BaseResponse<Boolean> updateBook(@RequestBody BookUpdateRequest bookUpdateRequest) {
        if (bookUpdateRequest == null || bookUpdateRequest.getId() == null) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        try {
            boolean result = bookService.updateBook(bookUpdateRequest);
            return BaseResponse.success(result);
        } catch (BusinessException e) {
            return BaseResponse.error(e.getCode(), e.getMessage());
        }
    ...
    @GetMapping("/get/{id}")
    public BaseResponse<Book> getBookById(@PathVariable("id") Long id) {
        if (id <= 0) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "ID不合法");
        }
        try {
    ...
    @GetMapping("/list")
    public BaseResponse<IPage<BookVO>> listBooks(BookQueryRequest bookQueryRequest) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(categoryStr)) {
                ...
                bookQueryRequest.setCategoryList(Arrays.asList(categories));
            }
            
            IPage<BookVO> bookPage = bookService.listBooksWithTags(bookQueryRequest);
            return BaseResponse.success(bookPage);
        } catch (BusinessException e) {
            return BaseResponse.error(e.getCode(), e.getMessage());
        }
    }
    ...
    public BaseResponse<List<String>> getCategories() {
        try {
            List<String> categories = bookService.getAllNonEmptyCategories();
    ...
    @GetMapping("/search")
    public BaseResponse<IPage<Book>> searchBooks(
        ...
        try {
            IPage<Book> bookPage = bookService.searchBooks(keyword, page, size);
            return BaseResponse.success(bookPage);
        } catch (BusinessException e) {
            return BaseResponse.error(e.getCode(), e.getMessage());
        }
    ...
    public BaseResponse<IPage<Book>> advancedSearch(
        ...
        try {
            IPage<Book> bookPage = bookService.advancedSearch(keyword, category, author, yearFrom, yearTo, page, size);
            return BaseResponse.success(bookPage);
        } catch (BusinessException e) {
            return BaseResponse.error(e.getCode(), e.getMessage());
        }
    ...
    public BaseResponse<IPage<Book>> templateSearch(
        ...
        try {
            IPage<Book> bookPage = bookService.templateSearch(field, value, page, size);
            return BaseResponse.success(bookPage);
        } catch (BusinessException e) {
            return BaseResponse.error(e.getCode(), e.getMessage());
        }
    ...
    public BaseResponse<IPage<Book>> nestedSearch(
            @Parameter(description = "搜索关键词") @RequestParam("keyword") String keyword,
        ...
        try {
            IPage<Book> bookPage = bookService.nestedSearch(keyword, maxYear, page, size);
            return BaseResponse.success(bookPage);
        } catch (BusinessException e) {
            return BaseResponse.error(e.getCode(), e.getMessage());
        }
    ...
    @GetMapping("/count")
    public BaseResponse<Long> countBooks(BookQueryRequest bookQueryRequest) {
        if (bookQueryRequest == null) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "请求参数为空");
        }
        try {
            long count = bookService.countBooks(bookQueryRequest);
    ...
    public BaseResponse<String> uploadBook(
    ...
    @GetMapping("/cover/{id}")
    public BaseResponse<String> getBookCover(
            @Parameter(description = "图书ID") @PathVariable("id") Long id) {
        if (id == null || id <= 0) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "图书ID不合法");
        }
        try {
            String base64Data = imgService.getImageByBookId(id);
            if (base64Data == null) {
    ...
    public BaseResponse<String> getBookCoverByImgId(
    ...
    public BaseResponse<String> extractText(
        ...
        // 先检查是否已存在章节
        if (bookService.hasBookSections(id)) {
    ...
    public BaseResponse<String> importBooksFromExcel(@RequestParam("file") MultipartFile file) {
        if (file == null || file.isEmpty()) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "文件不能为空");
        }
        ...
        try {
            ...
            CompletableFuture<String> future = CompletableFuture.supplyAsync(() -> {
                try {
                    ...
                    String importResult = bookService.importBooksFromExcel(fileContent, fileName);
                    
                    // 更新进度：处理封面
                    taskProgress.setProgress(50)
                        .setCurrentStep("获取需要处理封面的图书列表");
                    List<Book> booksWithoutCover = bookService.getBooksWithoutCover();
                    
                    int total = booksWithoutCover.size();
                    int current = 0;
                    
                    // 处理每本书的封面
                    for (Book book : booksWithoutCover) {
                        current++;
    ...
    @PostMapping("/opac/{id}")
    public BaseResponse<String> getBookOpacInfo(@PathVariable("id") Long id) {
        if (id == null || id <= 0) {
            return BaseResponse.error(ErrorCode.PARAMS_ERROR, "图书ID不合法");
        }
        ...
        // 检查是否已获取过OPAC信息
        if (book.getIsOpaced() != null && book.getIsOpaced() == 1) {
    ...
    public BaseResponse<String> batchGetOpacInfo() {
        ...
        CompletableFuture<String> future = CompletableFuture.supplyAsync(() -> {
            try {
                ...
                List<Book> books = bookService.getBooksWithoutOpac();
                ...
                for (Book book : books) {
                    ...
                    try {
                        String result = bookService.getAndSaveOpacInfo(book.getId());
    ...
    public BaseResponse<String> handleBatchExtractAndImport() {
        ...
        CompletableFuture<String> future = CompletableFuture.supplyAsync(() -> {
            try {
                // 获取未提取文本的图书列表
                List<Book> books = bookService.getBooksWithoutSections();
                int total = books.size();
                ...
                for (Book book : books) {
                    ...
                    try {
                        if (book != null) {
                            // 导入到ES
                            bookSectionEsService.importBookSections(book.getId());
